...
def get_gcc_matrices_2d(ksp_xyzc, ncc):
    ...
    raise NotImplementedError("Doesn't do gcc yet")
...
def align_cc_matrices(cc_matrices_init_xcc):
    xp = sp.get_array_module(cc_matrices_init_xcc)
    cc_matrices_aligned_xcc = xp.zeros_like(cc_matrices_init_xcc)
    cc_matrices_aligned_xcc[0, ...] = cc_matrices_init_xcc[0, ...]

    for xx in range(1, cc_matrices_init_xcc.shape[0]):
...
def get_gcc_matrices_3d(ksp_xyzc, ncc):
    ...
    cc_matrices_aligned_xcc = align_cc_matrices(cc_matrices_init_xcc)
