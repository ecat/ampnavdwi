    ...
    if phasenav_weighting_per_voxel_normalization == PhaseNavPerVoxelNormalizationType.MEAN_ACROSS_ALL_RESP_PHASES:
        ...
        if phasenav_weighting_per_voxel_normalization == PhaseNavPerVoxelNormalizationType.MEAN_PER_RESPIRATORY_PHASE:
            ...
            for rr, slicer in enumerate(resp_phase_slicer):
                ...
                scaling_per_vxyzr_[..., rr, :] = spatial_averaging_function(sorted_phasenav_weightings_vxyzrs[..., rr, 0:int(samples_to_normalize_against_per_r[rr])], 
        ...
        scaling_per_vxyzs = xp.nan_to_num(scaling_per_vxyzs, nan=1, posinf=1, neginf=1)
        scaling_per_vxyzs[scaling_per_vxyzs == 0] = 1
...
def walsh_method_new(im_cxyz, window_shape, window_stride, alg_str='eig'):
    # Walsh adaptive reconstruction of phased array MR imagery MRM 2000

    input_dim = im_cxyz.shape
    xp = sp.get_array_module(im_cxyz)

    if im_cxyz.ndim == 3:
        im_cxyz = xp.reshape(im_cxyz, input_dim + (1,))

    if len(window_shape) == 2:
        window_shape = window_shape + (1, )

    if len(window_stride) == 2:
        window_stride = window_stride + (1, )

    with sp.get_device(im_cxyz):
        ...
        def spatial_channel_to_channel_im(y_bc):
            # magnitude is taken when averaging overlappign blocks to avoid weird phase alignment
            spatial_channel_bsr = xp.tile(xp.abs(y_bc[..., np.newaxis]), (1, 1, spatial_channel_matrices_bcr.shape[-1]))
            spatial_channel_sbr = xp.transpose(spatial_channel_bsr, (1, 0, 2))
            blocks_cbbbwww = R1.H * spatial_channel_sbr
            return B.H * blocks_cbbbwww

        im_weightings_cxyz = spatial_channel_to_channel_im(spatial_channel_largest_vector_bc)        
        im_eigenvalues_cxyz = spatial_channel_to_channel_im(spatial_channel_eigenvalues) if alg_str == 'eig' else xp.zeros_like(im_weightings_cxyz)

        # flip eigenvalues so largest is first
        return xp.ascontiguousarray(xp.reshape(im_weightings_cxyz, input_dim)), xp.ascontiguousarray(xp.reshape(xp.flip(im_eigenvalues_cxyz, axis=0), input_dim))