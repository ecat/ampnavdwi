...
nv, nz, _, ns, nx, fse_etl, nc = ksp_vz_sxrc.shape
ny = sequence_opts["matrix_size"][1]
valid_shot_slicer_v = sequence_opts["valid_shots_range"]


ksp_vz_sxyc = extract_full_kspace(ksp_vz_sxrc, phase_encode_table_esv, ny)
ksp_cal_z_sxyc = np.squeeze(extract_full_kspace(kspace_cal_z_sxrc[np.newaxis, ...], phase_encode_table_cal_es[..., np.newaxis], ny), axis=0)
...
if do_fermi_filter:
    ...
    filter_along_y = lambda x: np.multiply(x, np.reshape(get_fermi_filter(ny), (1, 1, 1, 1, -1, 1)), x)
    filter_along_y(ksp_vz_sxyc)
...
nv, nz, _, ns, nx, ny, nc = ksp_vz_sxyc.shape
...
ksp_cal_sz_xyc, n_cal_shots = calibration_data_sorting(ksp_cal_z_sxyc, sequence_opts)

if is_jupyter:
    im_cal_sz_xyc = sp.ifft(ksp_cal_sz_xyc, axes=(-2, -3))
    im_cal_sz_xy = sos(im_cal_sz_xyc, axis_in=-1)
    im_cal_inner_product_z_xyc = im_cal_sz_xyc[0, ...] * np.conj(im_cal_sz_xyc[1, ...])
    sl_to_plot = 0
    pl.ImagePlot(im_cal_sz_xy[0, :, 0, ...], z=-3, x=-2, y=-1, title='cal 0')

# +
dcf_vz__xy_ = np.sum(mask_vz_sxy[..., np.newaxis], 3, keepdims=True)
dcf_vz__xy_[dcf_vz__xy_ == 0] = 1
ksp_naive_vz_xyc = np.sum(ksp_vz_sxyc / dcf_vz__xy_, axis=3)
im_naive_vz_xyc = sp.ifft(ksp_naive_vz_xyc, axes=(-3, -2))
im_naive_vz_xy = sos(im_naive_vz_xyc, axis_in=-1)

normalize_scale = np.max(np.abs(im_naive_vz_xy))
print('normalize scale', normalize_scale)
ksp_vz_sxyc = ksp_vz_sxyc / normalize_scale
...
if is_jupyter:
    ...
    for vv in range(nv):
        for sl in sl_to_show:
            im__xy_to_show = im_naive_vz_xy[vv, sl, ...]
            pl.ImagePlot(im__xy_to_show / np.max(im__xy_to_show), z=-3, x=-2, y=-1, 
...
if is_jupyter:
    for vv in range(nv):
        plt.figure()
        plt.subplot(211)
        plt.imshow(np.squeeze(np.abs(mask_vz_sxy[vv, 0, 0, ...])))
        plt.xlabel('shots')
        plt.ylabel('ky')
        plt.subplot(212)    
        plt.plot(np.squeeze(np.sum(np.abs(mask_vz_sxy[vv, 0, 0, ...]), 0)), 'x')
        plt.ylim([-.1, None])
        plt.show()
...
if do_cc:
    ksp_center_zxyc = sp.resize(ksp_cal_sz_xyc[0, :, 0, ...], (nz, ncalibx, ncaliby, nc))
    ksp_cc_vz_sxyc = np.zeros((nv, nz, 1, ns, nx, ny, ncc), dtype=xp.complex64)
    ksp_cc_cal_sz_xyc = np.zeros((n_cal_shots, nz, 1, nx, ny, ncc), dtype=xp.complex64)
        
    for z in range(nz):
        cc_matrix = get_cc_matrix(ksp_center_zxyc[z, ...], ncc, do_plot=False)
        ksp_cc_vz_sxyc[:, z, ...] = apply_cc_matrix(ksp_vz_sxyc[:, z, ...], cc_matrix)
        ksp_cc_cal_sz_xyc[:, z, ...] = apply_cc_matrix(ksp_cal_sz_xyc[:, z, ...], cc_matrix)

    ksp_vz_sxyc = sp.to_device(ksp_cc_vz_sxyc, recon_device)
    ksp_cal_sz_xyc = sp.to_device(ksp_cc_cal_sz_xyc, recon_device)
    print('Coil compression from ', str(nc), ' to ', str(ncc))
    nc = ncc
else:
    ksp_vz_sxyc = sp.to_device(ksp_vz_sxyc, recon_device)
    ksp_cal_sz_xyc = sp.to_device(ksp_cal_sz_xyc, recon_device)
    print('Skipped coil compression')


ksp_density_compensated_vzexyc = np.sum(sp.to_device(ksp_vz_sxyc, sp.cpu_device) / dcf_vz__xy_, axis=3) # keep on cpu to save memory
...
if sens_estimation_flag == SensitivityDataSource.FULL_LOW_BVALUE_IMAGE:
    ksp_for_sens_estimation_zcxy = np.transpose(ksp_density_compensated_vzexyc[0, :, 0, ...], (0, 3, 1, 2))
    ...
    ksp_for_sens_estimation_szcxy = np.transpose(ksp_cal_sz_xyc[:, :, 0, ...], (0, 1, 4, 2, 3)) 
...
ksp_cc_zxyc_b0 = sp.to_device(ksp_density_compensated_vzexyc[0, :, 0, ...], recon_device)
...
if is_jupyter:
    ...
    pl.ImagePlot(np.abs(im_conj_phase_zxy_b0), z=-3, x=-2, y=-1, vmax=.25*np.max(np.abs(im_conj_phase_zxy_b0)), title='density compensated conjugate phase coil combination')
...
im_phasenav_vz_sxy = xp.zeros((nv, nz, 1, ns, nx, ny), dtype=np.complex64)
...
for v in range(nv):
    ...
    ksp_z_sxyc = ksp_vz_sxyc[v, ...]
    # reconstuct only low-res data
    ksp_lowres_z_sxyc = sp.resize(ksp_z_sxyc, (nz, 1, ns, phasenav_recon_x, phasenav_recon_y, nc))    
    ksp_lowres_z_scxy = xp.ascontiguousarray(xp.transpose(ksp_lowres_z_sxyc, (0, 1, 2, 5, 3, 4)))

    for z in range(nz):
        sens_downsampled_cxy = sens_downsample_zcxy[z, ...]
        ksp_lowres_scxy = ksp_lowres_z_scxy[z, 0, ...]
        for shot in valid_shot_slicer: # only reconstruct valid shots
            ...
            im_phasenav_vz_sxy[v, z, 0, shot, ...] = sp.ifft(ksp_phasenav_lowres_xy, axes=(-1, -2))

im_phasenav_vzsxy = im_phasenav_vz_sxy[:, :, 0, ...] * im_mask_zxy[np.newaxis, :, np.newaxis, ...]
...
# +
# combine shots recon
im_vz_xy_recon = xp.zeros((nv, nz, 1, nx, ny), dtype=xp.complex64)
mask_vz_sxy = sp.to_device(mask_vz_sxy, recon_device)
mask_vzs_xy = xp.transpose(mask_vz_sxy, (0, 1, 3, 2, 4, 5))
...
for vv in range(0, nv):
    ...
    ksp_zcs_xy = xp.transpose(ksp_vz_sxyc[vv][:, :, valid_shot_slicer, ...], (0, 5, 2, 1, 3, 4))
    im_phasenav_zsxy = im_phasenav_for_recon_vzsxy[vv][:, valid_shot_slicer, ...]  
    mask_zs_xy = mask_vzs_xy[vv][:, valid_shot_slicer, ...]      
    ...
    im_vz_xy_recon[vv, ...], _ = fista_2d_fse_recon(ksp_zcs_xy, im_phasenav_zsxy, sens_zcxy_for_recon, mask_zs_xy, 
                                im_phasenav_magnitude_zsxy, max_iter=50, lamda=lambda_wav_independent)
# -


if is_jupyter:
    pl.ImagePlot(im_vz_xy_recon[0, 0::2, 0, ...].get(), z=-3, title='recon b0 images')

    for z in range(1, nz, 2):
        slicer_a = (0, z, 0, slice(None), slice(None))
        slicer_b = (1, z, 0, slice(None), slice(None))
        pl.ImagePlot(np.concatenate((im_vz_xy_recon[slicer_a].get(), 2 * im_vz_xy_recon[slicer_b].get()), axis=1), title='slice ' + str(z))

# generate adc map
diff_bval = sequence_opts["bvalue"] - sequence_opts["first_volume_bvalue"]
im_vzxy_recon = np.squeeze(np.abs(im_vz_xy_recon.get()))
...
cfl_exchanger.save_clinic_cfls(recon_out_dir, [im_vz_xy_recon.get(), adc_map_zxy])
...
cfl_exchanger.save_debug_cfls(recon_out_dir, [im_phasenav_vzsxy[vols_to_save, :, shots_to_save, ...].get(), 
                                                im_normalized_phasenav_weights_vzsxy[vols_to_save, :, shots_to_save, ...].get(), 
                                                im_phasenav_weights_vzsxy[vols_to_save, :, shots_to_save, ...].get(),
                                                sens_calzcxy.get()])
